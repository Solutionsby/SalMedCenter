...
export const ServicesPages: React.FC<priceObject> = ({
	content,
	isFirstService,
}) => {
	...
	return (
		<div className="services-pages-wrapper">
			...
			<div className="services-pages-descripon-wrapper">
				<div className="services-descripton">
					{t(`servicesContent.${content.id}.content`)}
				</div>
				...
				{isFirstService && currentLanguage != "en" && (
					<div className="first-time-visit-wrapper">
						<p className="description">{t("servicesContent.0.descripton")}</p>
						<DownloadForm />
					</div>
				)}
				<div className="service-component service">
					{filtredList.map(
						({ id, fontAwsomeIcon, descriptionKey, linkPath }) => (
							<div
								key={id}
								className="services-component-wrapper service-in-service">
